...
The only _truly_ band-limited signal that yields precisely a sampled impulse is the Sinc function, which has the downside of being infinitely long. See "Splitting the Unit Delay" by Laasko et al for more details and various approximation schemes that don't involve infinite-length signals.
