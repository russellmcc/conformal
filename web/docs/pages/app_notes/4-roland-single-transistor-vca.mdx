...
Lets look at a simplified diagram of the circuit (component values here taken from JX-8P):
...
$$I_B = I_S \left[\frac{1}{\beta_F}\left(e^{\frac{V_{BC}}{V_T}} - 1 \right) + \frac{1}{\beta_R}\left(e^{\frac{V_{BC}}{V_T}} - 1 \right)\right]$$
...
Note that since $V_{\text{in}}$ is small, this is approximately linear in $V_{\text{in}}$.

### $V_{BC}$ as a function of $V_{\text{control}}$

Since we don't want to work with the cumbersome Lambert's W function, we can make a somewhat crude approximation at least in the case where $V_{\text{control}} \gg V_{BC}$. In this case,
...
Here we verify the behavior of the circuit with an LTSpice simulation. ([download](/conformal/app-notes/4/simplified.asc))
