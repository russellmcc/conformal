...
<Callout type="warning">
  Note that this tutorial is written for macOS system. However, with small
  tweaks, it should work on Windows as well! See the [Quick
...
Next, we'll install VST SDK. This can live anywhere that's convenient for you - if you're not sure where to put it, why not in `~/SDKs`? Note that the VST SDK is currently only used for its validator tool.
